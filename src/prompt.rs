...
impl PromptStrategy for JsonPrompt {
    ...
    fn parse_response(&self, response: &str) -> Result<String> {
        let parsed_json: CommitMessageJson = serde_json::from_str(response)
            .with_context(|| format!("Failed to parse LLM response as JSON. Response: '{}'", response))?;
